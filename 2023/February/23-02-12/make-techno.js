...
function perc (mins) {
    let k = Math.round(Math.floor((4 * 30)) * mins)
    let h = Math.round(Math.floor((8 * 30)) * mins)
    let c = Math.round(Math.floor((2 * 30)) * mins)
    if (mins >= 1 && mins <= 100 && typeof mins === "number") return [`${k} kicks`, `${h} hihats`, `${c} claps`]
    else return "invalid track time"
};